...
def check_valid_number(number):
    ...
    # print(len(number_to_check) % 2)
    if not len(number_to_check) % 2:
        half_number = len(number_to_check) // 2
        first_part = number_to_check[:half_number]
        second_part = number_to_check[half_number:]
